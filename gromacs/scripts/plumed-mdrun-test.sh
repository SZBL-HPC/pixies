...
test_dir="$project_root/.pixi/plumed-gromacs/$environment/$version"
...
if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    export DYLD_LIBRARY_PATH="$plumed_prefix/lib${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH}"
else
...
printf '%s\n' \
    '; Minimal two-particle topology for the PLUMED smoke test.' \
    '[ defaults ]' \
    '1 2 yes 0.5 0.8333' \
    '' \
    '[ atomtypes ]' \
...
printf 'PLUMED GROMACS smoke test passed: %s (%s)\n' "$version" "$environment"
